...
// VND
// Referencia: Apostila - Inteligencia Computacional
vector<int> vnd(int **adj_matrix, vector <int> route)
{
	int r = 2; // numero de estruturas diferentes de vizinhança (swap e opt)
	int k = 1; // tipo de estrutura de vizinhanca corrente
	...
	while(k <= r)
	{
		...
		if(k == 1)
		{
			best_route = swap(adj_matrix, route);
			curr_cost = calculate_cost(adj_matrix, best_route);
		}

		// k = 2 (2-opt)
		else if(k == 2)
		{
			best_route = two_opt(adj_matrix, route);
			curr_cost = calculate_cost(adj_matrix, best_route);
		}

		// calcula custo pra rota original
		cost = calculate_cost(adj_matrix, route);
		
		// se custo calculado do melhor vizinho for MENOR que o da rota original
		if(curr_cost < cost)
		{
			// rota original recebe a rota de menor custo
			route = best_route;
			// como k começou com 1, continua sendo 1
			k = 1;
		}
